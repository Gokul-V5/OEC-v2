...
export const addProcedureUserToPlan = async (planId, procedureId,userId) => {
    const url = `${api_url}/Plan/AddProcedureUserToPlan`;
    ...
    if (!response.ok) throw new Error("Failed to create plan with users");

    return true;
};

export const getPlanUserProcedures = async (planId) => {
    const url = `${api_url}/UserPlanProcedure?$filter=planId eq ${planId}&$expand=procedure`;
    const response = await fetch(url, {
        method: "GET",
    });
...
export const deleteProcedureUserToPlan = async (planId, procedureId,userId) => {
    const url = `${api_url}/Plan/DeleteProcedureUserToPlan`;
    var command = { planId: planId, procedureId: procedureId, userId: userId };
    const response = await fetch(url, {
        method: "POST",
        headers: {
    ...
    if (!response.ok) throw new Error("Failed to delete plan with users");
